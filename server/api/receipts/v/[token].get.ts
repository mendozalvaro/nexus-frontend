...
export default defineEventHandler(async (event) => {
  ...
  if (!token || token.trim().length === 0) {
    throw createError({ statusCode: 400, statusMessage: "Token de verificacion requerido." });
  }
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    throw createError({ statusCode: 500, statusMessage: "Configuracion de verificacion incompleta." });
  }
