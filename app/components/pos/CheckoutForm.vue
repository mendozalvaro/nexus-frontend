  ...
  <UForm :schema="schema" :state="state" class="space-y-5" @submit="submit">
    ...
    <div class="grid gap-4 md:grid-cols-2">
      ...
      <UFormField label="Valor descuento" name="discountValue" class="md:col-span-2">
        <UInput v-model.number="state.discountValue" type="number" min="0" step="0.01" :disabled="loading || state.discountType === 'none'" class="w-full" />
      </UFormField>

      <UFormField label="Nota interna" name="note" class="md:col-span-2">
        <UTextarea v-model="state.note" :rows="4" placeholder="Referencia opcional para la venta o cobro." :disabled="loading" />
      </UFormField>
    </div>
