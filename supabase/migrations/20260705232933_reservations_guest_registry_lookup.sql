...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'guest_customers_sex_check'
  ) then
    alter table public.guest_customers
      add constraint guest_customers_sex_check
      check (sex is null or sex = any (array['male', 'female', 'other']));
  end if;
end $$;

create unique index if not exists idx_guest_customers_org_document
  on public.guest_customers (organization_id, document_type, document_number);
...
create index if not exists idx_reservation_guests_guest_customer
  on public.reservation_guests (guest_customer_id);
